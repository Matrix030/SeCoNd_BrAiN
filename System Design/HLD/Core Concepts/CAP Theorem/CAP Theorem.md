---
tags: [system-design, hld, cap-theorem, consistency, availability, distributed-systems]
aliases: ["CAP", "CAP Theorem", "Consistency Availability Partition Tolerance"]
---
...
- **Consistency**: 
	...
	- When a write is made to one node, all subsequent reads from any node will return that updated value.
- **Availability**: 
	- Every request to a non-failing node receives a response, without the guarantee that it contains the most recent version of the data.
- **Partition Tolerance**: 
...
> [!warning]
> Note that consistency in the context of the CAP theorem is quite different from the consistency guaranteed by [[ACID]] databases.
...
![[Pasted image 20260507160510.png]]
...
![[Pasted image 20260507160820.png]]
...
- **Distributed Transactions**: Ensuring multiple data stores (like [[Caching in System Design|cache]] and database) remain in sync through two-phase commit protocols. This adds complexity but guarantees consistency across all nodes. This means users will likely experience higher latency as the system ensures data is consistent across all nodes.
...
- **Multiple Replicas**: [[Sharding|Scaling]] to additional read replicas with asynchronous replication, allowing reads to be served from any replica even if it's slightly behind. This improves read performance and availability at the cost of potential staleness.
...
> [!warning]
> The following section covers more advanced concepts that might be relevant for senior and staff-level discussions.

1) As systems grow in complexity, the choice between consistency and availability isn't always binary. 
2) Modern distributed systems often require nuanced approaches that vary by feature and use case. Let's explore these advanced considerations.
3) Real-world systems frequently need both availability and consistency - just for different features. Let's look at two examples:
...
1) When discussing consistency in CAP theorem, people usually mean strong consistency - where all reads reflect the most recent write. 
2) However, understanding the spectrum of consistency models can help you make more nuanced design decisions:

	1) **Strong Consistency**: 
		- All reads reflect the most recent write. 
		- This is the most expensive consistency model in terms of performance, but is necessary for systems that require absolute accuracy like bank account balances. 
		- This is what we have been discussing so far.
	
	2) **Causal Consistency**: 
		- Related events appear in the same order to all users. 
		- This ensures logical ordering of dependent actions, such as ensuring comments on a post must appear after the post itself.
	
	3) **Read-your-own-writes Consistency**: 
		- Users always see their own updates immediately, though other users might see older versions. 
	...
	4) **Eventual Consistency**: 
		...
		- This is the most relaxed form of consistency and is often used in systems like DNS where temporary inconsistencies are acceptable.
...
- [[> Caching]] — caching strategies affected by consistency/availability tradeoffs
- [[Sharding]] — how data is partitioned across nodes in a distributed system
- [[Consistent Hashing]] — how distributed systems route requests across nodes
- [[Database Indexing]] — related data store fundamentals
